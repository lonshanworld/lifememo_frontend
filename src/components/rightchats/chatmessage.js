    ...
    return (
        <>
            ...
            className={`z-0 flex my-2 w-auto items-center flex-row ${props.currentuserId !== chat.userId ? "justify-start ml-3" : "justify-end mr-3"}`}>
                {props.currentuserId !== chat.userId && <img src={props.friendImg === "" ? Logo : props.friendImg } className="h-7 w-7 rounded-full" alt="profile"/>}
                <p 
                ...
                >{chat.text}</p>
                {props.currentuserId === chat.userId && <img src={props.selfImg === "" ? Logo : props.selfImg} className="h-7 w-7 rounded-full" alt="profile"/>}
            </div>)}
        </>
    );
}
